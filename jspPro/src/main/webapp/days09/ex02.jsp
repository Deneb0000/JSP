...
<div>
  <xmp class="code">
    ...
    => XML 데이터(JSON)를   자바스크립트언어 		 <- xml보다 jason이 더 많이 사용된다더 많이 씀
       비동기적으로 처리하는 기술.
    ...
    6. js ajax 처리 순서(과정)
       ...
       3) XMLHttpRequest 객체 '설정'.
           ㄱ. 요청 설정
              open() 메서드 - 요청 필요한 설정		//보내기 위해 앞서보내서 필요한 메드를 찾움
              send() 메서드 - 실제 서버에 요청감.		//가세요~
              
              A) get 방식		a작업이 끝나고 B 동기식 / B끝ㅌ나는중에 A 작동 : 비동기식
              XMLHttpRequest객체.open("GET", "/test.jsp?id=admin", true);
              XMLHttpRequest객체.send();					ㄴ ?뒤에주기
              B) post 방식
